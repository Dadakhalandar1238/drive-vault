...
and reassembled in offset order.
"""
...
def upload_many(clients: dict[str, DriveClient], payloads: list[tuple[str, bytes]]) -> dict[str, list[dict]]:
    """
    ...
    for filename, data in payloads:
        file_plans[filename] = plan_chunks(free_space, len(data))

    tasks = [
        (filename, data, acct, offset, chunk_size)
        for filename, data in payloads
    ...
    def do_task(task):
        ...
        file_id = clients[acct].fresh_copy().upload_bytes(chunk_name, chunk_bytes)
        return filename, {"account": acct, "file_id": file_id, "offset": offset, "size": chunk_size}

    grouped: dict[str, list[dict]] = {filename: [] for filename, _ in payloads}
    for filename, chunk in parallel_map(do_task, tasks):
        grouped[filename].append(chunk)
    return grouped


def download_with_chunks(clients: dict[str, DriveClient], chunks: list[dict]) -> bytes:
    ordered_chunks = sorted(chunks, key=lambda c: c["offset"])

    def fetch(chunk):
        return chunk["offset"], clients[chunk["account"]].fresh_copy().download_bytes(chunk["file_id"])

    results = parallel_map(fetch, ordered_chunks)
